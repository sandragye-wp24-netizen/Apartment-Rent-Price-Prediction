...
# ============================================================
    ...
    # --------------------------------------------------------
    # 5. BINARY FEATURES VS RENTAL PRICE
    # --------------------------------------------------------
    ...
    for column, label in binary_features.items():

        median_without = df.loc[
            df[column] == 0,
            "price"
        ].median()
    ...
    st.write(
        """
        This comparison examines whether apartment facilities
        and listing characteristics are associated with
        differences in median rental prices.

        Differences between the Yes and No groups suggest that
        these binary characteristics may provide useful
        information for rental-price prediction.
        """
    )
